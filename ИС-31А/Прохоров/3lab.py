...
while True:
    while True:
        try:
            input1 = int(input("Введите y координату: от 1 до "+str((rows-1))+"\n"))
            if input1 >= rows:
                print("Координата вне лабиринта")
            else:
                break
        except ValueError:
            print("Введите число")
    while True:
        try:
            input2 = int(input("Введите x координату: от 1 до "+str((columns-1))+"\n"))
            if input2 >= columns:
                print("Координата вне лабиринта")
            else:
                break
...
file2=open("D:/maze-for-me-done.txt",'w')
